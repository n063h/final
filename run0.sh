...
CUDA_VISIBLE_DEVICES=0 python train.py dataset=cifar model=resnet arch=pi arch.name=pi2 label_ratio=0.01 notes=pi2_works_cifar10 2>&1| tee log0_$(date +%y-%m-%d-%H-%M).txt
CUDA_VISIBLE_DEVICES=0 python train.py dataset=cifar model=resnet arch=pi arch.name=pi3 label_ratio=0.01 notes=pi3_timecost_cifar10 2>&1| tee log0_$(date +%y-%m-%d-%H-%M).txt

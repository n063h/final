...
# CE + MSE + MSE
# use mean prob of every set p/q to compute MSE loss 
class Arch(_Arch):
    ...
    def training_step(self, batch, batch_idx,optimizers):
        ...
        if self.conf.semi:
            ...
            u0+=(F.mse_loss(mixed_prob0[sup_num:],mixed_y[sup_num:,1,:])+F.mse_loss(mixed_prob0[sup_num:],mixed_y[sup_num:,2,:]))/2
            u1+=(F.mse_loss(mixed_prob1[sup_num:],mixed_y[sup_num:,0,:])+F.mse_loss(mixed_prob1[sup_num:],mixed_y[sup_num:,2,:]))/2
            u2+=(F.mse_loss(mixed_prob2[sup_num:],mixed_y[sup_num:,0,:])+F.mse_loss(mixed_prob2[sup_num:],mixed_y[sup_num:,1,:]))/2
